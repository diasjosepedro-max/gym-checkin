...
router.get('/users', require('../middleware/auth'), async (req, res) => {
  try {
    const { rows } = await db.query('SELECT id, email, name, created_at FROM users ORDER BY created_at');
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.delete('/users/:id', require('../middleware/auth'), async (req, res) => {
  try {
    if (req.user.id === req.params.id) return res.status(400).json({ error: 'Não podes remover a tua própria conta' });
