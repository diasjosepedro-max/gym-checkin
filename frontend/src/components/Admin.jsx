      ...
      {tab==='classes' && (
        <div>
          <div className="card">
            ...
            <div>{members.map(m=>{const on=ncForm.allowed_members.includes(m.id);return(<button key={m.id} className="tag-btn" onClick={()=>toggleMember(m.id)} style={on?{background:`${ncForm.color}22`,borderColor:`${ncForm.color}88`,color:ncForm.color}:{}}>{on?'✓ ':'+ '}{m.name}</button>);})}</div>
